...
>
> **State 3 — The reveal.** (a) One sentence of reflected understanding from `reasoning`, displayed as the headline — this is the "it heard me" moment, the product's emotional core. (b) A small SVG arc visualizing the 4 trajectory waypoints (valence→x, arousal→y), subtle. (c) The 4 tracks as cards: album art, name, artist, each linking to external_url; if preview_url exists, a 30s inline preview player. (d) THEN, below the value: "Save this as a playlist → Connect Spotify" button. OAuth comes after the payoff, never before.
>
